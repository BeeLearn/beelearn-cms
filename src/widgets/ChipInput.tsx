...
export default function ChipInput<T>({
  ...
  const handleInputEvent = function (
    ...
    timer = window.setTimeout(() => {
      ...
      if (onKeyUp) onKeyUp(event as React.KeyboardEvent<HTMLInputElement>);
    }, 500);
  };
  ...
  return (
    <div
      className="relative"
      onBlur={() =>
        (eventLoopTimer.current = window.setTimeout(() => setBlur(true), 500))
      }
    >
          ...
          {selected.map((value, index) => (
            ...
            >
              <button
                ...
                onClick={() => {
                  setSelected(
                    selected.filter((element) => !compare(element, value))
                  );
                }}
              >
                <MdClose />
      ...
      {!blur && (
        ...
        >
          {values
            ? values.length < 1
              ? emptyChild && emptyChild("empty")
              : values.map((value, index) => {
                  const exists = Boolean(
                    selected.find((element) => compare(element, value))
                  );
                  return (
                    <Fragment key={index}>
                      <div
                        ...
                        onClick={() => {
                          ...
                          else setSelected(selected.concat(value));
