...
function NavigationRouteItem({
  ...
  activeClassName,
  selectedIcon,
}: NavRouteItemProps & PropsWithClassName) {
  ...
  const active =
    path === "/"
      ? currentPath === "/" || currentPath.includes("/courses")
  ...
  return (
    ...
    >
      ...
      >
        ...
        <AccountPicker className="mx-2" user={userState.user!} />
        <div className="flex-1 flex flex-col pr-4 at-md:items-center at-md:p-0">
          {navigationRoutes
            .filter((route) => route.visibleWhen!(userState.user!))
            .map((item) => (
              <NavigationRouteItem
                key={item.path}
                {...item}
