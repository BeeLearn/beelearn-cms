...
import { TextInput, TextAreaInput, FileInput, InputToggle } from "@/widgets/TextInput";
...
type EditCourseDialogProps = {
    ...
    onSave: (course: Course, data: Partial<Course>, multipart: boolean) => Promise<Course>,
};
    ...
    function EditCourseDialog({ course, onSave, onDelete }, ref) {
        ...
        const searchTag = function (event: React.FormEvent<HTMLInputElement>) {
            ...
            if (value.trim().length > 1) {
                ...
                return Api.instance.tagController.list({
                    query: { search: value }
                }).then(({ data }) => {
                                ...
                                <div className="flex-1 flex flex-col space-y-4">
                                    {
                                        values.illustration && isURL(values.illustration) ?
                                            ...
                                            : <FileInput
                                                ...
                                                onReset={() => setFieldValue("illustration", course.illustration)} />
                                    }
                                    <TextInput
                                        name="name"
                                    ...
                                    <ChipInput
                                        ...
                                        emptyChild={(state) => <TagEmptyState state={state} fetchState={tagFetchState.current} />}
                                        onInput={searchTag}>
                                        {(value, selected) => (
                                            <div className={"flex space-x-2 items-center p-2 ".concat(selected ? "bg-violet-100 text-violet-900" : '')}>
                                                {
                                                    selected &&
                                    ...
                                    <FormSubmitButton
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                toast.error("Can't delete course, try again!");
                                            } finally {
                                                setIsDeleting(false);
                                            }
