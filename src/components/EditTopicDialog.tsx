...
export default forwardRef<Partial<DialogElement>, CreateNewTopicDialogProps>(
  function CreateNewTopicDialog({ topic, onSave, onDelete }, ref) {
    ...
    const [selectedTopicQuestion, setSelectedTopicQuestion] =
                  ...
                  >
                    ...
                    <button
                      onClick={(event) => {
                        ...
                        setFieldValue(
                          "topic_questions",
                          values.topic_questions.filter(
                            (element) => element !== topic_question
                          )
                        );
                      }}
                    >
              ...
              {mounted &&
                ...
                createPortal(
                  <EditQuestionDialog
                    ...
                    question={
                      selectedTopicQuestion.question as unknown as Question
                    }
                    onDelete={async (question) => {
                      const topicQuestions = values.topic_questions.filter(
                    ...
                    onSave={async (question, data) => {
                      const response =
                        await Api.instance.questionController.update({
                          path: question.id,
                          data,
                          query: {
